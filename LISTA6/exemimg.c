...
int main () {
   ...
   // testa int saveImgGray

    printf("\nTeste 1: saveImgGray\n");
    imgGray picture_file = alocaImagemGray(nLin, nCol);
    if (picture_file.img == NULL) {
        printf("\nErro ao alocar memoria para img Gray\n");
    }

    for (int i = 0; i < nLin; i++) {
        for (int j = 0; j < nCol; j++) {
            picture_file.img[i][j] = 15;
        }
    }

    char descricao[80] = "Teste 1 p/ imagem em tons de cinza";
    char nome_arquivo[50] = "imgGrayTeste1";

    if (saveImgGray(picture_file, descricao, nome_arquivo)) {
    ...
    freeImagemGray(&picture_file);

    // testa int saveImgRGB

    printf("\nTeste 2: saveImgRGB\n");
    imgRGB picture_file2 = alocaImagemRGB(nLin, nCol);

    for (int i = 0; i < nLin; i++) {
        for (int j = 0; j < nCol; j++) {
            picture_file2.img[i][j].R = 255;
            picture_file2.img[i][j].G = 255;
            picture_file2.img[i][j].B = 255;
        }
    }

    char descricao2[80] = "Teste 2 p/ imagem RGB";
    char nome_arquivo2[50] = "imgRGBTeste2";

    if (saveImgRGB(picture_file2, descricao2, nome_arquivo2)) {
        printf("Imagem salva com sucesso\n");
    }
    else {
        printf("Erro ao salvar imagem\n");
    ...
    freeImagemRGB(&picture_file2);

    // testa int loadHead

    printf("\nTeste 3: loadHead\n");
    char nome_arquivo3[50] = "imgRGBTeste2";
    char desc[80];

    int imgType = loadHead(nome_arquivo3, desc);
    ...
    if (imgType == 1) {
        ...
        printf("Erro ao carregar o cabecalho da imagem\n");
    }
